...
export const useMeta = ({
  ...
  const metaTags = [];
  const defaultOgImage = "https://res.cloudinary.com/dddzjiuet/image/upload/v1757509560/general/gfxkyhii2wupy7qjbivo.jpg";
  ...
  // Keywords for SEO
  metaTags.push({ name: 'keywords', content: 'collaboration, remote, teams, team, real-time, communication' });
  ...
  metaTags.push({ property: 'og:image:alt', content: `${ogTitle || title || 'Teem'} - Team collaboration platform` });
  ...
  metaTags.push({ name: 'twitter:image:alt', content: `${ogTitle || title || 'Teem'} - Team collaboration platform` });
